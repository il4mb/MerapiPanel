...
const assets_entry = function () {
    ...
    return glob.sync(path.join(working_dir, "Assets", "src", "*.{js,ts}"), {
        ...
        const key = path.resolve(working_dir, path.dirname(file).replace(/src$/, 'dist'), path.basename(file)).replace(/\.(js|ts)$/, '');
        const value = (path.resolve(working_dir, path.dirname(file), path.basename(file)).replace(/\.(js|ts)$/, ''));
...
const blocks_entry = function () {
    return glob.sync(path.join(working_dir, "Blocks", "**", "src", "*.{js,ts}"), {
        ignore: ['./node_modules/**', './dist/**', './.git/**', './.vscode/**', './.vscode-test/**', './vendor/**', './tests/**'], // Ignore node_modules directory
        nodir: true, // Treat directories as files
        maxDepth: 2 // Limit the depth of the search to 2 directory levels
    }).reduce((entry, file) => {
